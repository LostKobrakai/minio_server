defmodule MinioServer.VersionsClient do
  alias MinioServer.Config

  @doc """
  Create a json file listing versions of minio client based on their cdn and the checksums.

  Does only list versions 2020+, which are available in all `available_architectures()`.
  """
  def create_versions_file(path \\ "versions-client.json") do
  ...
  defp fetch_release_versions_available_in_all_architectures do
    for arch <- Config.available_architectures() do
      url = "https://dl.min.io/client/mc/release/#{arch}/archive/"
      ...
      for {<<"mc.RELEASE.", version::binary-size(20)>>, _file} <- map,
          match?(<<year::binary-size(4), _::binary>> when year >= "2020", version),
          Map.has_key?(map, "mc.RELEASE.#{version}.sha256sum"),
          into: MapSet.new() do
        version
      end
    end
  ...
  defp fetch_checksums(versions) do
    ...
    |> Task.async_stream(fn {version, arch} ->
      url = url_for_release(:client, arch, version) <> ".sha256sum"

      {:ok, {200, body}} =
        :httpc.request(:get, {String.to_charlist(url), []}, [], full_result: false)

      [checksum, <<"mc.RELEASE.", ^version::binary-size(20)>>] =
