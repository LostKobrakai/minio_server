  ...
  * `:version` - Specify the version to download. Defaults to most recent.
  ...
  defp handle_downloading(arch, version, filename, checksum, opts) do
    ...
    case check_filename_status(filename, force) do
      :exists ->
        Logger.info("Download of minio server binary for #{arch} skipped: already exists.")
        :exists
      ...
      task ->
        case task do
          :replace ->
            ...
            Logger.info(
              "Download of minio server binary for #{arch} (version #{version}): Replacing existing."
            )

          :download ->
            Logger.info("Download of minio server binary for #{arch} (version #{version}).")
        end
